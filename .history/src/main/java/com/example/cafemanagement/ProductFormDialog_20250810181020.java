...
private void populateFields(Product p) {
    ...
    if (savedImagePath != null && !savedImagePath.isEmpty()) {
        ...
        if (imgFile.exists()) {
            ...
            // Defensive: check if image width and height are valid
            int width = dropLabel.getWidth();
            int height = dropLabel.getHeight();

            if (width <= 0) width = 200;  // fallback width
            if (height <= 0) height = 150; // fallback height
            ...
            if (img != null && img.getWidth(null) > 0 && img.getHeight(null) > 0) {
                ...
                dropLabel.setText("<html><center>Invalid Image</center></html>");
                dropLabel.setIcon(null);
            }
        } else {
  ...
  private void onSave() {
    try {
        ...
        // Temporary popup to debug calls
        if (product == null) {
            JOptionPane.showMessageDialog(this, "Inserting product: " + name);
            product = new Product(null, name, price, category, savedImagePath);
            productDAO.insert(product);
        } else {
            JOptionPane.showMessageDialog(this, "Updating product: " + name);
