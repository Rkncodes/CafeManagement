...
title.setForeground(Color.BLACK); // You can change this color to whatever you want
top.add(title, BorderLayout.WEST);
   ...
   private JComponent createCartPanel() {
    ...
    cart.setPreferredSize(new Dimension(400, 0));
    cart.setBorder(BorderFactory.createCompoundBorder(
            BorderFactory.createLineBorder(new Color(180, 180, 180), 1),
            new EmptyBorder(15, 15, 15, 15)
    ));
    cart.setBackground(new Color(245, 245, 245));
    ...
    // Add table number display
    JLabel tableLabel = new JLabel("Table: 1");
    tableLabel.setFont(new Font("SansSerif", Font.PLAIN, 14));
    tableLabel.setForeground(new Color(70, 70, 70));
    headerPanel.add(tableLabel, BorderLayout.EAST);
    
    cart.add(headerPanel, BorderLayout.NORTH);

    // Cart items with improved styling
    cartItemsPanel = new JPanel();
    cartItemsPanel.setLayout(new BoxLayout(cartItemsPanel, BoxLayout.Y_AXIS));
    cartItemsPanel.setBorder(new EmptyBorder(10, 0, 10, 0));
    ...
    totalLabel = new JLabel("<html><b>Subtotal:</b> ₹0.00<br>" +
                          "<b>Tax (10%):</b> ₹0.00<br>" +
                          "<b style='font-size:14px'>Total:</b> <b style='font-size:16px;color:#2E7D32'>₹0.00</b></html>");
    ...
    footerPanel.add(checkoutBtn, BorderLayout.SOUTH);
    cart.add(footerPanel, BorderLayout.SOUTH);
   ...
   private void refreshCartUI() {
    ...
    if (cartManager.isEmpty()) {
        JLabel emptyLabel = new JLabel("<html><center>Your cart is empty<br>Add items to get started</center></html>");
        ...
        for (Map.Entry<Product, Integer> entry : cartManager.getItems().entrySet()) {
            ...
            // Left side: Product name and price per unit
            JPanel leftPanel = new JPanel(new BorderLayout());
            JLabel nameLabel = new JLabel(product.getName());
            JLabel priceLabel = new JLabel(nf.format(product.getPrice()) + " each");
            ...
            minusBtn.setFont(new Font("SansSerif", Font.BOLD, 10));
            ...
            plusBtn.setFont(new Font("SansSerif", Font.BOLD, 10));
            ...
            JLabel totalPriceLabel = new JLabel(nf.format(product.getPrice() * qty));
            ...
            rightPanel.add(totalPriceLabel, BorderLayout.CENTER);
    ...
    // Update total label with tax information
    String totalText = String.format("<html><b>Subtotal:</b> %s<br>" +
                                    "<b>Tax (10%%):</b> %s<br>" +
                                    "<b>Total:</b> %s</html>",
            nf.format(cartManager.getTotal()),
            nf.format(cartManager.getTax()),
            nf.format(cartManager.getGrandTotal()));
...
private void showCheckoutDialog() {
    ...
    // Customer info form with improved layout
    ...
    JSpinner tableSpinner = new JSpinner(new SpinnerNumberModel(1, 1, 20, 1));
    ...
    gbc.gridx = 1;
    formPanel.add(tableSpinner, gbc);
    ...
    summary.setFont(new Font(Font.MONOSPACED, Font.PLAIN, 14));
...
private void showEnhancedInvoice(String invoiceText) {
    JDialog invoiceDialog = new JDialog(this, "Invoice", true);
    ...
    JTextArea invoiceArea = new JTextArea(invoiceText);
    invoiceArea.setEditable(false);
    invoiceArea.setFont(new Font(Font.MONOSPACED, Font.BOLD, 14));
    invoiceArea.setBackground(new Color(240, 240, 240));
    invoiceArea.setBorder(new EmptyBorder(20, 20, 20, 20));
    
    JPanel buttonPanel = new JPanel(new FlowLayout(FlowLayout.CENTER, 10, 10));
    buttonPanel.setBorder(new EmptyBorder(10, 10, 10, 10));
    
    JButton printBtn = new JButton("Print Receipt");
    printBtn.setIcon(UIManager.getIcon("FileView.fileIcon"));
    printBtn.addActionListener(e -> {
        try {
            invoiceArea.print();
        } catch (Exception ex) {
    ...
    JButton closeBtn = new JButton("Close");
    closeBtn.setIcon(UIManager.getIcon("OptionPane.errorIcon"));
    closeBtn.addActionListener(e -> invoiceDialog.dispose());
