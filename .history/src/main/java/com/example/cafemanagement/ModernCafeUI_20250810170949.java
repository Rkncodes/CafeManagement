...
public class ModernCafeUI extends JFrame {
    ...
    private void reloadProducts(String q, String category) {
        productsGrid.removeAll(); // Always clear before adding

        Set<Integer> addedIds = new HashSet<>(); // Prevent duplicate IDs
        ...
        for (Product p : list) {
            ...
            if (!addedIds.contains(p.getId())) { // only add if not already in grid
